...
import { Link } from "react-router-dom";import './login.css'
import ReactPasswordToggleIcon from "react-password-toggle-icon";import * as api from './api';
...
export default function LenderRegister() {
    ...
    const [registrationField, setRegistrationField] = useState({
      ...
      const handleLenderRegister = async () => {
        ...
        const validationError = api.validateRegisterInput(registrationField.email, registrationField.password ,registrationField.moblie);
  ...
  const Otpverify = async () => {
    try {
      ...
      console.error('Error:', error.response ? error.response.data.errorMessage : error.message);
      setError(error.response ? error.response.data.errorMessage : 'An error occurred during OTP validation');
    }
  };
  ...
  return (
    <div>
        ...
        <div className="login-wrapper">
         ...
         {submitotp ? <>     <div  className="maincircle"><div className="circle"><i class="fa-solid fa-user-check"></i></div></div>
              <div className="cend"><h2   className="textcenter">You are one step away from completing registration.</h2>  <hr />
              <p className="textcent">An activation link has been sent to your registered e-mail. Please check your inbox and activate your OxyLoans account to start Lending</p>  
              </div>

         </> : <> {field   ? <>  <h1>Register as a Lender</h1></>  : <><h1>Enter the otp sent to your mobile number</h1></>}  </>}    
                  ...
                  <>   
                      ...
                      <label>
                      NAME AS PER PANCARD <span className="login-danger">*</span>
                      </label>
                      <input className="form-control" type="text" name="name" onChange={handlechange}/>
                      <span className="profile-views">
                        <i className="fas fa-user-circle" />
                      </span>{registrationField.pancarderror && <div className="error">{registrationField.pancarderror}</div>}
                    </div>
                    <div className="form-group">
                      <label>
                      ...
                      </span>{registrationField.emailerror && <div className="error">{registrationField.emailerror}</div>}
                    </div>
                    <div className="form-group">
                      <label>
                      ...
                      {/* <input className="form-control pass-input" type="text" />
                                            <span className="profile-views feather-eye toggle-password">
                                                <FeatherIcon icon="eye" />
                                            </span> */}     {registrationField.passworderror  && <div className="error">{registrationField.passworderror}</div>}
                    </div>
                    <p className="reffertext">If you are referred by an existing lender,Please enter his/her referrer id ( EX : LR100001)</p>
                             
                    <div className="form-group">
                      <label>
                      ENTER THE REFERRER ID <span className="login-danger">*</span>
                      </label>
                      {/* <input className="form-control pass-confirm" type="text" /> */}
                      <input
                        ref={inputRef2}
                        className="form-control pass-confirm"
                        type="text" name="referrerId"   
                        onChange={handlechange}
                      />

                      <span className="profile-views">
                        <i className="fas fa-phone" />
                      </span>    {registrationField.referrerIderror && <div className="error">{registrationField.referrerIderror}</div>}
                    </div>
                    <div className="form-group">
                      <label>
                      ...
                      <input
                        ref={inputRef2}
                        className="form-control pass-confirm"
                        type="text"  name="moblie"   
                        onChange={handlechange}
                      />

                      <span className="profile-views">
                        <i className="fas fa-phone" />
                      </span> {registrationField.moblieerror && <div className="error">{registrationField.moblieerror}</div>}
                    </div>
                   {error  &&  <div className="errormessage">{error}</div>}
                    ...
                    <div className="form-group mb-0">
                      <button
                        ...
                        type="submit"        onClick={handleLenderRegister}> 
                    ...
                    <>  
                                    {submitotp  ? <></> : <>  <div className="maincircle">
                  <div className="circle">  <i class="fa-solid fa-user-lock"></i></div></div>   
